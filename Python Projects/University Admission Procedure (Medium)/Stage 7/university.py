class Applicant:
    def __init__(self, first_name, last_name, physics_exam, chemistry_exam, math_exam, cs_exam, admit_test,
                 first, second, third):
        ...
        self.admit_test = int(admit_test)
        self.first = first
        self.second = second
        self.third = third
...
class Department:
    exam_lookup = {"Physics": ["Physics", "Mathematics"], "Chemistry": ["Chemistry"], "Mathematics": ["Mathematics"],
                   "Engineering": ["Computer Science", "Mathematics"], "Biotech": ["Chemistry", "Physics"]}
    ...
    def applicant_score(self, applicant):
        return max(sum(applicant.scores[subject] for subject in Department.exam_lookup[self.name]) \
               / len(Department.exam_lookup[self.name]), float(applicant.admit_test))
    ...
    def accept(self, applicants, rank):
        ...
        if num_spaces == 0:
            ...
            dept_applicants = sorted(dept_applicants,
                                     key=lambda x: (-self.applicant_score(x), x.name()))
    ...
    def __str__(self):
        str_list = []
        str_list.extend([x.name() + " " + str(self.applicant_score(x))
                         for x in
                         sorted(self.accepted, key=lambda x: (-self.applicant_score(x), x.name()))])
        return '\n'.join(str_list)
...
for dept in dept_list:
    with open(f"{dept.name.lower()}.txt", "w") as file:
        file.write(dept.__str__())
    print(dept.name)
    print(dept)
    print()
