...
class Department:
    ...
    def applicant_mean(self, applicant):
        return sum(applicant.scores[subject] for subject in Department.exam_lookup[self.name]) \
               / len(Department.exam_lookup[self.name])
    ...
    def accept(self, applicants, rank):
        ...
        if num_spaces == 0:
            ...
            dept_applicants = sorted(dept_applicants,
                                     key=lambda x: (-self.applicant_mean(x), x.name()))
    ...
    def __str__(self):
        str_list = []
        str_list.extend([x.name() + " " + str(self.applicant_mean(x))
                         for x in
                         sorted(self.accepted, key=lambda x: (-self.applicant_mean(x), x.name()))])
        return '\n'.join(str_list)
