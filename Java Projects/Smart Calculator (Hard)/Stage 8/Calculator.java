...
public class Calculator {
    ...
    private final HashMap<String, BigInteger> variables;
    ...
    public void execute() {
        while (true) {
            ...
            BigInteger result = parsePostix(postfix);
        ...
        if (containsAssignment) {
            ...
            for (int i = args.size() - 1; i >= 0; --i) {
                if ("=".equals(args.get(i))) {
                    ...
                    if (postfix == null)
                        return;
                    BigInteger rhs = parsePostix(postfix);
                    if (rhs == null)
                        return;
                    else {
    ...
    private BigInteger parsePostix(ArrayList<String> postfix) {
        ArrayDeque<BigInteger> stack = new ArrayDeque<>();
        for (String s : postfix) {
            //unary operators first
            if ("#".equals(s) || "@".equals(s)) {
                BigInteger top = stack.pollFirst();
                switch (s) {
                    case "@":
                        stack.offerFirst(top);
                        break;
                    case "#":
                        stack.offerFirst(top.multiply(new BigInteger(Integer.toString(-1))));
                        break;
                }
            } else if (isOperator(s)) {
                BigInteger second = stack.pollFirst();
                BigInteger first = stack.pollFirst();
                switch (s) {
                    case "*":
                        stack.offerFirst(first.multiply(second));
                        break;
                    case "/":
                        ...
                        stack.offerFirst(first.divide(second));
                        break;
                    case "+":
                        stack.offerFirst(first.add(second));
                        break;
                    case "-":
                        stack.offerFirst(first.subtract(second));
                        break;
                }
            } else {
               if (isInteger(s))
