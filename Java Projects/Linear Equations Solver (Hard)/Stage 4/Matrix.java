...
public class Matrix {
    ...
    public void toReducedRowEchelonForm() {
        ...
        for (int i = 0; i < nRows; ++i) {
            final int row = i;
            if (matrix.get(i).get(nCols - 1) != 0 && IntStream.range(0, nCols - 1).allMatch(j -> matrix.get(row).get(j) == 0)) {
                isSolvable = false;
        ...
        int numNonzeroRows = matrix.stream().mapToInt(a -> a.stream().anyMatch(d -> d != 0) ? 1 : 0).sum();;
