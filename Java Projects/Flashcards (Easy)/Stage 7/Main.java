...
public class Main {
    public static void main(String[] args) {
        ...
        //Handle case of command line args
        String export = null;
        if (args.length != 0) {
            for (int i = 0; i < args.length; i++) {
                if ("-import".equals(args[i]) && i < args.length - 1) {
                    String imp = args[i + 1];
                    deck.importCardsFromFile(imp);
                } else if ("-export".equals(args[i]) && i < args.length - 1) {
                    export = args[i + 1];
                }
            }
        }

        //Basic interactive runtime activity
        while (deck.handleInput()) {
            deck.logThenPrint("");
        }

        //Handle possible export command
        if (export != null) {
            deck.exportCardsToFile(export);
        }
    }
}
...
class FlashCardDeck {
    ...
    public boolean handleInput() {
        ...
        if ("add".equals(command)) {
            ...
            importCardsFromFile(null);
            return true;
        } else if ("export".equals(command)) {
            exportCardsToFile(null);
            return true;
